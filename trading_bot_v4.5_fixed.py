#!/usr/bin/env python3
"""
Trading Bot v4.5.1 Ultra Pro
40 actifs : 20 Crypto + 20 Actions
Bougies 4h + Confluence multi-timeframe (4h + 1j)
Filtre anti-doublon intelligent (cooldown 4h)
2 scans séparés : Crypto (non-stop) + Actions (heures ouvertes)
...
# ── 20 Cryptos (marché 24h/24) ──────────────────────────────────────────────
WATCHLIST_CRYPTO = [
    "BTC-USD", "ETH-USD", "SOL-USD", "BNB-USD", "XRP-USD",
    "ADA-USD", "AVAX-USD", "DOT-USD", "POL28321-USD", "LINK-USD",
    "UNI7083-USD", "ATOM-USD", "LTC-USD", "DOGE-USD", "SHIB-USD",
    "APT21794-USD", "ARB11841-USD", "OP1-USD", "INJ-USD", "SUI20947-USD"
]

# ── 20 Actions (NYSE/NASDAQ + Europe) ───────────────────────────────────────
WATCHLIST_STOCKS = [
    "NVDA", "AAPL", "MSFT", "META", "GOOGL",
    "AMZN", "AMD", "TSM", "ORCL", "CRM",
    "TSLA", "NFLX", "BABA", "JPM", "GS",
    "V", "BRK-B", "MC.PA", "ASML", "SAP"
]

WATCHLIST = WATCHLIST_CRYPTO + WATCHLIST_STOCKS

SIGNAL_THRESHOLD      = 0.70
SCAN_INTERVAL_MINUTES = 240  # Scan toutes les 4h (aligné avec les bougies 4h)

# Fuseaux horaires des marchés
...
def get_model_path(symbol: str) -> Path:
    return MODELS_DIR / f"{symbol.replace('-','_').replace('.','_')}.pkl"

def save_model(symbol: str, models: dict):
    try:
        joblib.dump(models, get_model_path(symbol))
        logger.info(f"💾 Modèle sauvegardé : {symbol}")
    except Exception as e:
        logger.warning(f"Impossible de sauvegarder {symbol}: {e}")
...
def load_model(symbol: str):
    ...
    if path.exists():
        try:
            ...
            logger.info(f"⚡ Modèle chargé depuis disque : {symbol}")
            return models
        except Exception as e:
            logger.warning(f"Impossible de charger {symbol}: {e}")
    return None

# ═══════════════════════════════
# DÉTECTION HORAIRES DE MARCHÉ
# ═══════════════════════════════

def is_market_open(symbol: str) -> bool:
    """Retourne True si le marché de l'actif est ouvert."""
    now_ny    = datetime.now(TZ_NY)
    now_paris = datetime.now(TZ_PARIS)
    weekday   = now_ny.weekday()  # 0=Lundi, 6=Dimanche

    # Crypto : ouvert 24h/24, 7j/7
    if symbol in WATCHLIST_CRYPTO:
        return True

    # Actions européennes (MC.PA, ASML, SAP)
    european = ["MC.PA", "ASML", "SAP"]
    if symbol in european:
        if weekday >= 5: return False
        h = now_paris.hour + now_paris.minute / 60
        return 9.0 <= h <= 17.5  # 09h00 – 17h30 Paris

    # Actions US (NYSE / NASDAQ) : 09h30 – 16h00 NY, lun–ven
    if weekday >= 5: return False
    h = now_ny.hour + now_ny.minute / 60
    return 9.5 <= h <= 16.0

def market_status_str(symbol: str) -> str:
    if symbol in WATCHLIST_CRYPTO:
        return "24h/24 🟢"
...
def detect_candle_pattern(df):
    ...
    body   = abs(c - o)
    range_ = h - l + 1e-9
    if body / range_ < 0.1:                                   return "Doji ⚖️",           "neutral"
    if c > o and c2 < o2 and c > o2 and o < c2:              return "Bullish Engulfing 🟢","bullish"
    if c < o and c2 > o2 and c < o2 and o > c2:              return "Bearish Engulfing 🔴","bearish"
    if c > o and (o - l) > 2 * body:                         return "Hammer 🔨",           "bullish"
    if c < o and (h - o) > 2 * body:                         return "Shooting Star ⭐",    "bearish"
    if c > o:                                                 return "Bougie Haussière 🟢", "bullish"
    return "Bougie Baissière 🔴", "bearish"

def add_features(df):
    df = df.copy()
    # Correction yfinance multi-level columns
...
def train_ensemble(df):
    ...
    if len(np.unique(y)) < 2:
        raise ValueError(f"Données insuffisantes : {len(np.unique(y))} classe(s) uniquement")
    sc = MinMaxScaler()
...
def get_technical_analysis(df):
    ...
    bullish_signals = sum([
        rsi < 30, macd_bull, bb_pos < 0.2,
        above_ema20, above_ema50, above_ema200,
        golden, candle_type == "bullish"
    ])
    return {
        "rsi": rsi, "rsi_label": rsi_label, "rsi_em": rsi_em,
        "macd_label": macd_label, "macd_em": macd_em, "macd_val": round(last["macd"], 4),
...
def get_news_sentiment(symbol):
    ...
    q = symbol.replace("-USD", "").replace("-", " ").replace(".PA", "")
    try:
        arts = requests.get(
...
def get_order_book(symbol):
    ...
    try:
        ...
        return {
            "buy_ratio": round(bv/tot*100,1), "sell_ratio": round(sv/tot*100,1),
...
def compute_piotroski(info):
    ...
    try:
        ...
        if info.get("sharesOutstanding",0) > 0:    score += 1
...
def build_signal(symbol, confidence, votes, df):
    ...
    extra.append(ta["bullish_signals"]/8)
    return {
...
def fmt(s):
    ...
    now = datetime.now(TZ_KL).strftime("%d/%m/%Y à %H:%M")
    nv  = sum(1 for v in s["votes"].values() if v>=0.5)
    tf  = s.get("timeframe", "1j")
    rev = s.get("reversal_alert", "")
    L = [
        "╔══════════════════════════════════════════╗",
        f"{s['cem']}  SIGNAL {s['action']} — {s['symbol']} [{tf}]",
        "╚══════════════════════════════════════════╝","",
    ]
    if rev: L += [f"🔄 {rev}", ""]
    L += [
        f"💰 Prix actuel   : ${s['price']:,.2f}",
        f"{cem} Variation {tf}  : {s['change24']:+.2f}%","",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
    ...
    if s.get("confluence") is not None:
        c1d_pct = int(s.get("conf_1d", 0)*100)
        a1d = s.get("action_1d", "?")
        conf_em = "✅" if s["confluence"] else "⚠️"
        L += [
            "","━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            "⏱️  CONFLUENCE MULTI-TIMEFRAME",
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            f"  📊 Bougie 4h  : {s['action']}  🟢  (signal principal)",
            f"  📊 Bougie 1j  : {a1d}  {'🟢' if a1d=='BUY' else '🔴' if a1d=='SELL' else '🟡'}  (tendance, {c1d_pct}%)",
            f"  {conf_em} {s['confluence_label']}",
        ]
    ...
    if ta["vol_surge"]: L.append("  🔥 Volume       : Surge détecté !")
    bull = ta["bullish_signals"]
    L.append(f"  📊 Score TA     : {bull}/8  {'🟢'*bull}{'⚪'*(8-bull)}")
    ...
    if s["fd"] and s["fd"]["de"] and s["fd"]["de"]>2: warns.append(f"→ Endettement élevé (D/E={s['fd']['de']})")
    if s["fd"] and s["fd"]["piotroski"] and s["fd"]["piotroski"]<4: warns.append("→ Score Piotroski faible — santé financière fragile")
    if s["ob"] and s["ob"]["resistance"]: warns.append(f"→ Résistance forte à ${s['ob']['resistance']:,.0f}")
    if s["fg"] and s["fg"]["value"]>=75: warns.append("→ Extreme Greed — surveiller une correction")
    if s["macro"] and s["macro"]["dxy"]>0.5: warns.append("→ Dollar en hausse — pression baissière")
    if s["ta"]["rsi"]>70: warns.append(f"→ RSI en surachat ({s['ta']['rsi']}) — risque de correction")
    if warns: L+=["","⚠️  POINTS DE VIGILANCE :"]+[f"  {w}" for w in warns]
    L+=["",f"⏱️  Analysé le : {now}",f"🔁  Prochain scan : {s.get('timeframe', '1j')} (toutes les 4h)"]
    return "```\n"+"\n".join(L)+"\n```"
...
trained_models = {}
last_signals = {}  # {symbol: {"action": "BUY", "confidence": 0.81, "time": datetime}}

async def analyze_and_send(symbol, channel, force=False):
    try:
        logger.info(f"Analyse {symbol}...")

        # ── Données 4h (signal principal) ──────────────────────────────
        df_4h = yf.download(symbol, period="90d", interval="1h", progress=False, auto_adjust=True)
        ...
        if len(df_4h) < 50: return
        df_4h = add_features(df_4h)

        # ── Données 1j (filtre de tendance) ────────────────────────────
        df_1d = yf.download(symbol, period="6mo", interval="1d", progress=False, auto_adjust=True)
        if df_1d.empty or len(df_1d) < 50: return
        if isinstance(df_1d.columns, pd.MultiIndex):
            df_1d.columns = df_1d.columns.get_level_values(0)
        df_1d = add_features(df_1d)

        # ── Entraînement / cache ML ─────────────────────────────────────
        monday = datetime.now().weekday() == 0
        model_key = f"{symbol}_4h"
        if model_key not in trained_models:
            cached = load_model(model_key)
            if cached and not monday:
                trained_models[model_key] = cached
            else:
                logger.info(f"Entraînement {symbol} 4h...")
                trained_models[model_key] = train_ensemble(df_4h)
                save_model(model_key, trained_models[model_key])
        elif monday and model_key not in getattr(bot, "_retrained_today", set()):
            logger.info(f"Réentraînement lundi {symbol}...")
            trained_models[model_key] = train_ensemble(df_4h)
            save_model(model_key, trained_models[model_key])
            if not hasattr(bot, "_retrained_today"): bot._retrained_today = set()
            bot._retrained_today.add(model_key)

        confidence, votes = predict_ensemble(trained_models[model_key], df_4h)
        if 0.30 <= confidence <= 0.70:
            logger.info(f"{symbol}: HOLD ignoré ({confidence:.2f})"); return

        # ── Confluence 4h + 1j ─────────────────────────────────────────
        conf_1d, _ = predict_ensemble(trained_models[model_key], df_1d) if len(df_1d) >= 50 else (confidence, {})
        action_4h = "BUY" if confidence >= 0.65 else ("SELL" if confidence < 0.30 else "HOLD")
        action_1d = "BUY" if conf_1d   >= 0.65 else ("SELL" if conf_1d   <= 0.45 else "HOLD")
        confluence = action_4h == action_1d and action_4h != "HOLD"
        confluence_label = "✅ CONFLUENCE VALIDÉE — Signal renforcé" if confluence else "⚠️ Pas de confluence — Signal faible"

        # ── Filtre anti-doublon (cooldown 4h) ─────────────────────────
        now = datetime.now(TZ_KL)
        prev = last_signals.get(symbol)
        if prev and not force:
            elapsed_h = (now - prev["time"]).total_seconds() / 3600
            same_action = prev["action"] == action_4h
            small_conf_change = abs(confidence - prev["confidence"]) < 0.10
            if elapsed_h < 4 and same_action and small_conf_change:
                logger.info(f"{symbol}: doublon ignoré ({action_4h}, {elapsed_h:.1f}h écoulées)")
                return
            if same_action and small_conf_change:
                logger.info(f"{symbol}: même signal sans changement significatif — ignoré")
                return

        # ── Alerte retournement ────────────────────────────────────────
        reversal_alert = ""
        if prev and prev["action"] != action_4h and action_4h != "HOLD":
            reversal_alert = f"🔄 RETOURNEMENT : {prev['action']} → {action_4h}"

        # Sauvegarder le signal
        last_signals[symbol] = {"action": action_4h, "confidence": confidence, "time": now}

        signal  = build_signal(symbol, confidence, votes, df_4h)
        ...
        signal["confluence_label"] = confluence_label
        signal["action_1d"]        = action_1d
        signal["conf_1d"]          = round(conf_1d, 2)
        signal["reversal_alert"]   = reversal_alert
        signal["timeframe"]        = "4h"

        message = fmt(signal)
        for chunk in [message[i:i+1900] for i in range(0, len(message), 1900)]:
            await channel.send(chunk)
        logger.info(f"✅ {action_4h} — {symbol} ({int(confidence*100)}%) confluence={confluence}")
    except Exception as e:
        logger.error(f"Erreur {symbol}: {e}")

# ── Scan Crypto : toutes les 60 min, 24h/24 ─────────────────────────────────
@tasks.loop(minutes=SCAN_INTERVAL_MINUTES)
async def scan_crypto():
    channel = bot.get_channel(DISCORD_CHANNEL_ID)
    if not channel: return
    now = datetime.now(TZ_KL).strftime("%H:%M")
    logger.info(f"=== Scan CRYPTO {now} ===")
    await channel.send(f"```\n🪙 SCAN CRYPTO — {now} (20 actifs)\n```")
    for symbol in WATCHLIST_CRYPTO:
        await analyze_and_send(symbol, channel)
        await asyncio.sleep(3)

# ── Scan Actions : toutes les 60 min, seulement si marché ouvert ─────────────
@tasks.loop(minutes=SCAN_INTERVAL_MINUTES)
async def scan_stocks():
    channel = bot.get_channel(DISCORD_CHANNEL_ID)
    if not channel: return
    open_stocks = [s for s in WATCHLIST_STOCKS if is_market_open(s)]
    if not open_stocks:
        logger.info("Marché fermé — scan actions ignoré")
        return
    now = datetime.now(TZ_KL).strftime("%H:%M")
    logger.info(f"=== Scan ACTIONS {now} ({len(open_stocks)} actifs ouverts) ===")
    await channel.send(f"```\n📈 SCAN ACTIONS — {now} ({len(open_stocks)}/20 marchés ouverts)\n```")
...
async def on_ready():
    ...
    channel = bot.get_channel(DISCORD_CHANNEL_ID)
    if channel:
        crypto_list = " | ".join(WATCHLIST_CRYPTO)
        stocks_list = " | ".join(WATCHLIST_STOCKS)
        await channel.send(
            f"```\n🤖 Trading Bot v4.5.1 Ultra Pro démarré !\n\n"
            f"🧠 ML        : RF + GB + SVM + XGBoost + LightGBM\n"
            f"📈 Tech      : RSI + MACD + Bollinger + EMA + Chandelier\n"
            f"🏦 Fondamentaux : Piotroski + FCF + ROE + D/E\n"
            f"📰 News      : VADER + TextBlob\n\n"
            f"🪙 20 CRYPTOS (scan 24h/24) :\n{crypto_list}\n\n"
            f"📈 20 ACTIONS (scan heures ouvertes) :\n{stocks_list}\n\n"
            f"🔁 Scans toutes les {SCAN_INTERVAL_MINUTES} min\n"
            f"🕐 Marchés US  : 09h30–16h00 NY (lun–ven)\n"
            f"🕐 Marchés EU  : 09h00–17h30 Paris (lun–ven)\n```")
        if not scan_crypto.is_running(): scan_crypto.start()
        if not scan_stocks.is_running(): scan_stocks.start()
...
@bot.command(name="marches")
async def cmd_marches(ctx):
    lines = ["```", "📊 STATUT DES MARCHÉS", ""]
    lines.append("🪙 CRYPTO (toujours ouvert)")
    for s in WATCHLIST_CRYPTO:
        lines.append(f"  {s:<12} {market_status_str(s)}")
    lines.append("")
...
async def cmd_watchlist(ctx):
    await ctx.send(
        f"```\n📋 WATCHLIST (40 actifs)\n\n"
        f"🪙 20 Cryptos :\n{chr(10).join(WATCHLIST_CRYPTO)}\n\n"
        f"📈 20 Actions :\n{chr(10).join(WATCHLIST_STOCKS)}\n```")

@bot.command(name="status")
async def cmd_status(ctx):
    ns = SCAN_INTERVAL_MINUTES - (datetime.now().minute % SCAN_INTERVAL_MINUTES)
    open_s = sum(1 for s in WATCHLIST_STOCKS if is_market_open(s))
    await ctx.send(
        f"```\n🤖 TRADING BOT v4.5.1 Ultra Pro\n\n"
        ...
        f"🪙 Cryptos actifs : 20/20 🟢\n"
        f"📈 Actions ouvertes: {open_s}/20\n"
        f"🔁 Prochain scan  : dans ~{ns} min\n```")

@bot.command(name="scancrypto")
async def cmd_scancrypto(ctx):
    now = datetime.now(TZ_KL).strftime("%H:%M")
    await ctx.send(f"```\n🪙 SCAN MANUEL CRYPTO — {now}\n⏳ 20 cryptos en cours... (~15 min)\n```")
    for sym in WATCHLIST_CRYPTO:
        await analyze_and_send(sym, ctx.channel)
        await asyncio.sleep(3)
    await ctx.send("```\n✅ Scan crypto terminé !\n```")

@bot.command(name="scanactions")
async def cmd_scanactions(ctx):
    open_stocks = [s for s in WATCHLIST_STOCKS if is_market_open(s)]
    now = datetime.now(TZ_KL).strftime("%H:%M")
    if not open_stocks:
        await ctx.send("```\n⚠️ Marchés fermés — aucune action à scanner.\nHoraires : US 09h30-16h00 NY | EU 09h00-17h30 Paris\n```")
        return
    await ctx.send(f"```\n📈 SCAN MANUEL ACTIONS — {now}\n⏳ {len(open_stocks)}/20 marchés ouverts... (~{len(open_stocks)*20//60+1} min)\n```")
    for sym in open_stocks:
        await analyze_and_send(sym, ctx.channel)
        await asyncio.sleep(3)
    await ctx.send("```\n✅ Scan actions terminé !\n```")

@bot.command(name="scanall")
async def cmd_scanall(ctx):
    open_stocks = [s for s in WATCHLIST_STOCKS if is_market_open(s)]
    total = len(WATCHLIST_CRYPTO) + len(open_stocks)
    now = datetime.now(TZ_KL).strftime("%H:%M")
    await ctx.send(
        f"```\n🔍 SCAN MANUEL COMPLET — {now}\n"
        f"🪙 20 cryptos + 📈 {len(open_stocks)}/20 actions ouvertes\n"
        f"⏳ {total} actifs — durée estimée : ~{total*20//60+1} min\n```")
    for sym in WATCHLIST_CRYPTO:
        await analyze_and_send(sym, ctx.channel)
        await asyncio.sleep(3)
    if open_stocks:
...
@bot.command(name="signaux")
async def cmd_signaux(ctx):
    now = datetime.now(TZ_KL)
    if not last_signals:
        await ctx.send("```\n⚠️ Aucun signal envoyé depuis le démarrage.\n```"); return
    lines = ["```", f"📋 DERNIERS SIGNAUX ({len(last_signals)} actifs)", ""]
    for sym, data in sorted(last_signals.items()):
        elapsed = (now - data["time"]).total_seconds() / 3600
        em = "🟢" if data["action"]=="BUY" else ("🔴" if data["action"]=="SELL" else "🟡")
        lines.append(f"  {sym:<12} {em} {data['action']:<5} {int(data['confidence']*100)}%  il y a {elapsed:.1f}h")
    lines.append("```")
    await ctx.send("\n".join(lines))

@bot.command(name="cache")
async def cmd_cache(ctx):
    cached = list(MODELS_DIR.glob("*.pkl"))
    total_mb = sum(f.stat().st_size for f in cached) / 1e6
    lines = ["```", f"💾 CACHE MODÈLES ML — {len(cached)}/40 actifs", ""]
    for sym in WATCHLIST:
        key = sym.replace("-","_").replace(".","_")
        exists = (MODELS_DIR / f"{key}.pkl").exists()
        loaded = sym in trained_models
        status = "✅ disque+RAM" if (exists and loaded) else ("💾 disque" if exists else ("🧠 RAM seul" if loaded else "❌ absent"))
        lines.append(f"  {sym:<12} {status}")
    lines += ["", f"📦 Taille totale : {total_mb:.1f} MB", "```"]
    await ctx.send("\n".join(lines))
...
async def cmd_aide(ctx):
    await ctx.send(
        ...
        "!scancrypto       → Scan manuel des 20 cryptos\n"
        "!scanactions      → Scan manuel des actions (si marché ouvert)\n"
        "!scanall          → Scan manuel complet (40 actifs)\n"
        "!signaux          → Derniers signaux envoyés + heure\n"
        "!top5             → Analyse les 5 premiers actifs\n"
        "!cache            → Statut du cache ML (disque + RAM)\n"
        "!watchlist        → Liste des 40 actifs\n"
        "!marches          → Statut ouvert/fermé de chaque marché\n"
...
if __name__ == "__main__":
    ...
    logger.info("Démarrage Trading Bot v4.5.1 Ultra Pro...")
