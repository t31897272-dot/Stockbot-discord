...
async def analyze_and_send(symbol, channel):
    try:
        ...
        df_raw = yf.download(symbol, period="6mo", interval="1d", progress=False, auto_adjust=True)
        if df_raw.empty or len(df_raw) < 50: return
        # Aplatir colonnes multi-niveaux yfinance
        if isinstance(df_raw.columns, pd.MultiIndex):
            df_raw.columns = df_raw.columns.get_level_values(0)
        df = add_features(df_raw)
        monday = datetime.now().weekday() == 0
        if symbol not in trained_models:
            # Essayer de charger depuis disque d'abord
            cached = load_model(symbol)
            if cached and not monday:
                trained_models[symbol] = cached
            else:
                logger.info(f"Entraînement {symbol}...")
                trained_models[symbol] = train_ensemble(df)
                save_model(symbol, trained_models[symbol])
        elif monday and symbol not in getattr(bot, "_retrained_today", set()):
            # Réentraînement hebdomadaire le lundi
            logger.info(f"Réentraînement lundi {symbol}...")
            trained_models[symbol] = train_ensemble(df)
            save_model(symbol, trained_models[symbol])
            if not hasattr(bot, "_retrained_today"):
                bot._retrained_today = set()
            bot._retrained_today.add(symbol)
...
async def on_ready():
    ...
    if channel:
        ...
        await channel.send(
            f"```\n🤖 Trading Bot v4.4 Ultra Pro démarré !\n\n"
...
if __name__ == "__main__":
    ...
    logger.info("Démarrage Trading Bot v4.4 Ultra Pro...")
